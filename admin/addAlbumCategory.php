...
if(isset($_POST["addCategory"]) && isset($_POST["category"]) && isset($_FILES["thumbnail"])) {
    ...
    $sql = "SELECT id FROM albumCategories WHERE title=:category";
    ...
    if($query->rowCount() != 0) {
        ...
        $query = $con->prepare("SELECT AUTO_INCREMENT FROM information_schema.Tables where TABLE_SCHEMA='ail' and TABLE_NAME='albumcategories'");
        $query->execute();
        $newCategoryId = $query->fetchColumn();
        $dpUploadDirPath = "entities/categories/albumCategories/". $newCategoryId .DIRECTORY_SEPARATOR;
            ...
            if($category_thumbnail_size > $maxThumbnailSize) {
                ...
                if(file_exists($category_thumbnail_upload_path)) {
                    ...
                    if(!move_uploaded_file($category_thumbnail_tmpname, $category_thumbnail_upload_path)) {
                        ...
                        CategoryProvider::addCategoryToDatabase($con, $categoryTitle, $dpUploadDirPath.$category_thumbnail_name, 2); // 2 for entity type album
...
<html>
    ...
    <title>Welcome! Add Album Category</title>
            ...
            <div class="settingsContainer column" style="width:100%;">
                <div class="uploadForm">
                    <form method="post" enctype="multipart/form-data">
                        <h2 style="margin: 0;">Add An Album Category</h2>
                        <div class="formData">
